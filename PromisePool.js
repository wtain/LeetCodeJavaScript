/**
 * https://leetcode.com/problems/promise-pool/?utm_campaign=DailyD13&utm_medium=Email&utm_source=Daily&gio_link_id=z98mwE0R
 * 
...
Beats
31.3% */
// https://leetcode.com/problems/promise-pool/editorial/?utm_campaign=DailyD13&utm_medium=Email&utm_source=Daily&gio_link_id=z98mwE0R
// var promisePool = async function(functions, n) {
...
//         await evaluateNext();
//     }
//     const promises = Array(n).fill().map(evaluateNext);
...
var promisePool = async function(functions, n) {
    ...
    return await Promise.all(functions.slice(0, n).map(f => f().then(evaluateNext)));
};
...
tests.forEach(async test => {
    await promisePool(test.functions, test.n);
    console.log("Done");
});